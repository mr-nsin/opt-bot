...
## 1. Architecture Preservation (The Core Wrapper)
**Rule:** NEVER attempt to rewrite the Python sidecar from scratch (e.g. replacing it with direct `ib_insync` or asyncio loops). 
* **Reason:** The established `BOT.py` / `tws_api_client.py` class-based wrapper contains complex, battle-tested options logic (contract scanning, delta-based strikes, trailing TP/SL, SQLite logging, EOD close-out). A rewrite leaves the engine feature-incomplete and broken for actual order routing.
* **Practice:** Always build on top of the `TradingEngine` wrapper class that instantiates `tws_api_client.py` and coordinates with `BOT.py` globals.

## 2. State Management & IPC Chain (The Sacred Flow)
...
## 4. Options Position Matching Key
